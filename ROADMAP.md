...
- [~] **Methods / OOP** — `fn <type>::<method>(...)` definitions keyed to a type, structs foremost: the explicit qualified-call foundation, the `recv.method(args)` dot-call sugar, the `S(args)` constructor sugar, method inheritance through `extends` (with the required `@override` marker), and dynamic dispatch through [polymorphic base views](docs/language.md#polymorphic-base-views) — fat `&base` references carrying the dispatch table, objects keeping their exact byte layout
