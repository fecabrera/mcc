...
fn show<T: int32 | int16 | int8>(x: T) {
    println(f"signed   {x as int32}");
}

fn show<T: uint32 | uint16 | uint8>(x: T) {
    println(f"unsigned {x as uint32}");
}
...
fn main() -> int32 {
    ...
    println("width(0) = {}  (the group's default anchored the literal)".format(
            width(0)));
    return 0;
}
