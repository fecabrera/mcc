...
fn main() -> int32 {
    ...
    println(f"start = ({start.x}, {start.y})");
    ...
    *p = start;             // copy the value in...
    ...
    println(f"sizeof(point) = {sizeof(struct point)}");
    ...
    println("sizeof(node<int32>) = {}, alignof = {}, next @ {}".format(
    ...
    let q = *p;
    q.x = 30;
    println(f"q = ({q.x}, {q.y}), p untouched = ({p->x}, {p->y})");
    dealloc(p);
