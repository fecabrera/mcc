...
// struct elements: `const` erases from a function type, so a by-value
// comparator inhabits it at every T; see functions/reference_callbacks.mc.)
...
fn main() -> int32 {
    ...
    println(f"entry.second = {value_of(e)}");
    println(f"value_of(pair) = {value_of(make_pair(7))}");

    // The comparator alias, reassignable like any function value.
    let choose: cmp<int32> = less;
    println(f"pick(less, 3, 9) = {pick(choose, 3, 9)}");
    choose = greater;
    println(f"pick(greater, 3, 9) = {pick(choose, 3, 9)}");

    // Inert parameter: both spellings are the one `int32` type, so they add.
    let a: boxed<bool> = 20;
    let b: boxed<char> = 22;
    println(f"boxed<bool> + boxed<char> = {a + b}");

    // Defaulted alias: `record` alone is `record<int64>`.
    let r: record;
    r.second = 100;
    println(f"record.second = {r.second}");
