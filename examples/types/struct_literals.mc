...
fn main() -> int32 {
    ...
    println(f"segment ({seg.from.x},{seg.from.y}) -> ({seg.to.x},{seg.to.y})");
    ...
    println("config: capacity={} verbose={} name={}".format(cfg.capacity, cfg.verbose, cfg.name));
    ...
    println(f"default config: capacity={dfl.capacity} verbose={dfl.verbose}");
    ...
    *h = point { x = 10, y = 20 };
    ...
    print(cmd.name);
    println(f": {cmd.args.length} args");   // "ls: 3 args"
