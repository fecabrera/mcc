...
fn main() -> int32 {
    ...
    println(f"p = ({p.x}, {p.y}, {p.z})");

    // The base is a true prefix: x and y sit at the front, z after them.
    println("offsetof(point3, z) = {}, sizeof(point3) = {}".format(
    ...
    println(f"length2 = {length2(&p as struct point*)}");
    ...
    let c = db_config { };
    println(f"capacity = {c.capacity}, verbose = {c.verbose}, port = {c.port}");

    // The specializations share scalar's exact layout...
    println("sizeof: scalar {}, meters {}, feet {}".format(
    ...
    println(f"doubled = {m.value} meters");
