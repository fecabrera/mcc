...
fn describe<T>(v: T) {
    println("%s: %d bytes", typename(T), sizeof(T) as int32);
}
...
fn show(a: any) {
    case type (a) {
        ...
        when T* ptr:
            println("a boxed %s (pointee %s)", typename(ptr), typename(T));
        when T v:
            println("a boxed %s", typename(T));
        else:
...
fn main() -> int32 {
    ...
    println("%s", typename(int32**));       // int32**
    println("%s", typename(slice<int32>));  // slice<int32>
    println("%s", typename(box<int64>));    // box<int64>
