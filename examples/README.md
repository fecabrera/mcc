...
| [formatting.mc](systems/formatting.mc) | the stdlib `format` overload set: direct calls appending each value's rendering into a `string`, the modifier grammars (integer `[0][width][x|X|b|p]`, float `[[N].M]f`, string `[N][s][N]`, bool `"yes"`), per-element slice rendering (nesting recurses), the `<typename>` fallback, and one user overload making a struct printable |
