...
| [error_handling.mc](types/error_handling.mc) | recoverable errors as values: the nominal `error` declaration (auto-numbering from 1 with explicit values continuing the count, display strings as data, truthiness against the reserved zero state, `==`/`!=`, `case`, the `as int32` read-out and nothing else), the builtin `result<T, E>` / `result<E>` with the explicit, context-typed `ok()`/`error()` constructors, and the binding forms that consume one: the `let ret, err = f();` destructure (falsy no-error state on ok, zero value of `T` on error), `try f() except (err) { ... }` with an `emit` fallback or a diverging handler, `else` as the ok arm (skipped on the fallback path), the unary-level `try` composing as an operand, the obligation-free statement-position handler over `result<E>`, and the `return error(err)` propagation idiom |
