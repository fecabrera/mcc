...
| [iteration.mc](iteration.mc) | `for x in` over the iter/next protocol (array, set, dict), the builtin `iterator<T>`/`pair<K, V>` structs, `break`/`continue`, bare `{ }` block scopes |
