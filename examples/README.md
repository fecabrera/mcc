...
methods with their dot-call sugar, constructors, `extends`
bases with the method families they hand down, flexible array members),
...
| [method_alias.mc](types/method_alias.mc) | methods through `type` aliases and on BUILTIN types: the qualifier is chased to the canonical name, so alias and canonical spellings declare and call ONE family; a plain alias qualifier (`fn pointf::magnitude` with `type pointf = point<float64>`) IS the specialization, a generic alias with written arguments substitutes through its target (`fn swap<int32, U>::pick` with `type swap<X, Y> = pair<Y, X>` IS the partial `fn pair<U, int32>::pick`), a duplicate-position alias (`type diag<T> = pair<T, T>`) is a diagonal constraint matching only `pair<X, X>` (falling through to a generic sibling on mismatch, winning the [subsumption](functions/overload_subsumption.mc) tie-break against it on an agreeing receiver), a bare generic-alias qualifier cannot declare (annotate it, `fn pt<T>::mk`; a fully-defaulted alias is the one bare exception, and calls chase by name so they stay bare), alias spellings in signatures stay transparent to inference, and builtins are qualifiers too (`fn int32::clamp`, `fn slice<T>::first`, callable through an alias as one family, never specializable) |
| [method_calls.mc](types/method_calls.mc) | the `.method()` call sugar: `recv.method(args)` desugaring to `Type::method(recv, args)` with the receiver passed verbatim (overloads, `mut`-receiver legality, and diagnostics the desugared call's own; the qualified spelling staying valid); fields shadowing methods (a fn-typed field keeps the field call, the method reachable qualified, only neither-field-nor-method getting the combined error, no bound-method values), a pointer receiver auto-dereffing exactly one hop (`->` stays fields-only, `S**` an error), rvalue receivers evaluating once into a hidden const local (chaining works, mut-self on a temporary rejected, a mut-RETURNING receiver re-lending its lvalue), mut-returning dot calls as lvalues (`l.at(0) = 9`, `+=`, chained store targets, formation chaining), generic receivers inferring `T` (type arguments never spelled at a dot call), builtin receivers (`'q'.upper()` via std/char, a declared `slice<T>::first`), and dot calls in f-string holes |
| [constructors.mc](types/constructors.mc) | the constructor call sugar: a method named `constructor` makes its type callable, `S(args)` desugaring to exactly `let s: S; S::constructor(s, args);` (the slot default-initialized first, `let` binding it directly with no copy, diagnostics the family call's own); every head form -- a bare non-generic struct, explicit type arguments `point<float64>(1, 1)` pinning `T` so a converting `constructor<U>` beats the int-literal-rejecting diagonal, a BARE generic head `point(1.5, 2.5)` inferring the instantiation from the arguments (int literals leaning int32), a transparent alias `pointf(3, 4)`, a fully-defaulted generic constructing at its defaults, and a builtin (`char(65)`) once someone declares its family; expression/return positions, the desugared spelling staying first-class, no-constructor calls WITH arguments erroring (never a cast) while struct literals remain the no-constructor spelling, and the implicit empty constructor: `T()` with zero arguments as exactly `let t: T;` (field defaults in, a declared arg-taking family not suppressing it, a receiver-only member claiming it as the declared non-fallback, builtin heads included) |
