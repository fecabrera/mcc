...
| [error_handling.mc](types/error_handling.mc) | recoverable errors as values, stage 1: the nominal `error` declaration (auto-numbering from 1 with explicit values continuing the count, display strings as data, truthiness against the reserved zero state, `==`/`!=`, `case`, the `as int32` read-out and nothing else), the builtin `result<T, E>` / `result<E>` carrying either the ok value or the error, and the explicit, context-typed `ok()`/`error()` constructors; results returned, passed, stored in locals and struct fields, still opaque until the binding forms land |
