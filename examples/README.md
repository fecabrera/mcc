...
| [tuples.mc](types/tuples.mc) | `tuple<A, B, ...>`, the ad-hoc unnamed product type: paren literals `(a, b)` with per-position struct-literal coercion, multiple return values as the headline, compile-time-constant bounds-checked indexing with lvalue elements, constant slicing `t[n:m]` copying positions out into a new smaller tuple (any arity: the 1-tuple `(x,)` and the zero-sized empty tuple `()`), `len(t)` arity recovery folding into constant index/slice bounds, uninitialized declaration and whole-value copies, `const` params by hidden reference, generic inference through the shape, a `type` alias naming a shape, arrays of tuples, struct layout under `sizeof`, the `<tuple<...>>` println fallback |
