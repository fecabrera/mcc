...
fn main() -> int32 {
    ...
    println("view: length {}, sum {}", view.length, sum(view));   // 3, 6

    // Argument positions included -- no named array, no temporary binding.
    println("inline: {}", sum([10, 20, 30] as slice<const int32>)); // 60
    ...
    println("adapted: nums[2] {}, sum {}", nums[2], sum(nums));     // 255, 302
    ...
    println("m rows: sum {}, sum {}", sum(m[0]), sum(m[1]));        // 3, 7

    let jagged = [[5, 6], [7]] as slice<slice<int32>>;
    for row in jagged {
        println("row of {}: sum {}", row.length, sum(row));       // 2: 11, 1: 7
    }

    // The char seam: a literal's length is its EXACT element count, with no
    // NUL logic at all.
    let hi = ['h', 'i'] as slice<char>;
    println("literal chars: length {}", hi.length);               // 2
    ...
    println("named char[2]: length {}", named.length);            // 1
    ...
    println("empty: length {}, sum {}", empty.length, sum(empty)); // 0, 0
    ...
    s[0] = 9;
    println("after s[0] = 9: sum {}", sum(s));                      // 11

    // The @static view declared at the top of the file.
    println("static table: length {}, sum {}", table.length, sum(table)); // 3, 60
