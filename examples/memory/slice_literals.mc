...
fn main() -> int32 {
    ...
    println(f"view: length {view.length}, sum {sum(view)}");   // 3, 6

    // Argument positions included -- no named array, no temporary binding.
    println(f"inline: {sum([10, 20, 30] as slice<const int32>)}"); // 60
    ...
    println(f"adapted: nums[2] {nums[2]}, sum {sum(nums)}");     // 255, 302
    ...
    println(f"m rows: sum {sum(m[0])}, sum {sum(m[1])}");        // 3, 7

    let jagged = [[5, 6], [7]] as slice<slice<int32>>;
    for row in jagged {
        println(f"row of {row.length}: sum {sum(row)}");       // 2: 11, 1: 7
    }

    // The char seam: a literal's length is its EXACT element count, with no
    // NUL logic at all.
    let hi = ['h', 'i'] as slice<char>;
    println(f"literal chars: length {hi.length}");               // 2
    ...
    println(f"named char[2]: length {named.length}");            // 1
    ...
    println(f"empty: length {empty.length}, sum {sum(empty)}"); // 0, 0
    ...
    s[0] = 9;
    println(f"after s[0] = 9: sum {sum(s)}");                      // 11

    // The @static view declared at the top of the file.
    println(f"static table: length {table.length}, sum {sum(table)}"); // 3, 60
