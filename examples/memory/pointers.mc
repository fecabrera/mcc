...
fn main() -> int32 {
    ...
    *p = 99;
    ...
    println("sizeof(uint8)  = {}", sizeof(uint8));
    println("sizeof(int32*) = {}", sizeof(int32*));
    ...
    println("sizeof(x)      = {}", sizeof(x));    // x is int32 -> 4
    println("sizeof(p)      = {}", sizeof(p));    // p is int32* -> 8

    // `as` casts: between integer types (truncate / extend), to and from
    // float64, and between pointer types.
    println("300 as uint8     = {}", 300 as uint8);
    println("-1 as int64      = {}", -1 as int64);
    println("2 as float64     = {}", 2 as float64);
    println("3.99 as int32    = {}", 3.99 as int32);
    
    let bytes = nums as byte*;   // `byte` is the alias for uint8: raw memory
    let addr_is_zero = (0 as uint8*) as uint64 == 0;
    println("null addr test   = {}", addr_is_zero);
