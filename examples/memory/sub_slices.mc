...
fn main() -> int32 {
    ...
    println(f"mid:  length {mid.length}, sum {sum(mid)}");     // 3, 90
    println(f"tail: length {tail.length}, sum {sum(tail)}");   // 3, 120
    println(f"head: length {head.length}, sum {sum(head)}");   // 2, 30
    println(f"all:  length {all.length}, sum {sum(all)}");     // 5, 150
    ...
    mid[0] = 99;
    println(f"after mid[0] = 99: nums[1] is {nums[1]}");           // 99

    // Sub-slices chain -- each step narrows the previous view...
    println(f"nums[1:][1:] starts at {nums[1:][1:][0]}");          // 30
    ...
    let start: int32 = 1;
    println(f"int32 start: sum {sum(nums[start:])}");              // 219
    ...
    let none = nums[2:2];
    println(f"empty: length {none.length}, sum {sum(none)}");  // 0, 0
    ...
    println(f"first {first}, nums[1] is now {nums[1]}");       // 10, 21
    ...
    println(f"borrowed array tail: sum {sum((arr as slice<int32>)[1:])}"); // 9
    ...
    let flag = true;
    println(f"ternary start: sum {sum(nums[flag ? 1 : 2 : 4])}");  // 91
