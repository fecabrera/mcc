...
cat > "$WORK/hi.mc" <<'MC'
import "std/io";
fn main() -> int32 { println("packaged ok: {}", 30); return 0; }
MC
cd "$WORK" || exit 1

run_echo "$VENV/bin/mcc" hi.mc -Wall -Werror --run   # JIT: codegen + the bundled stdlib
run_echo "$VENV/bin/mcc" hi.mc -Wall -Werror -o hi   # native: object emission and linking
run_echo ./hi
