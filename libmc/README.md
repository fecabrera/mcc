...
| [ring.mc](ring.mc)                     | `import "ring";`           | Growable generic FIFO ring buffer `ring<T>`: `ring_init`/`ring_destroy`, `ring_push`/`ring_pop`/`ring_peek`/`ring_at`, `ring_len`/`ring_is_empty` (doubles when full, re-laying wrapped elements in logical order).                                                                       |
...
| [string.mc](string.mc)                 | `import "string";`         | Growable text string `string` (a `type string = list<char>`): `string_init`/`string_destroy`/`string_reset`, `string_get`/`string_set`, `string_push` (one char), `string_append`/`string_eq`, and `string_it`/`string_next` iteration. `string_init` and `string_append` are overloaded on their source: a `const slice<char>` copies any borrowed run (a string borrows in with `as`, a literal adapts directly), a NUL-terminated `char*` copies to the terminator, a `(char*, n)` pair copies exactly n bytes.  |
