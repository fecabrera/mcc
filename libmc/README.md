...
| Module                                 | Import                     | Provides                                                                                                                                                                                                                                                                                  |
| -------------------------------------- | -------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [list.mc](list.mc)                     | `import "list";`           | Growable generic `list<T>`: `list_init`/`list_destroy`/`list_reset`, `list_get`/`list_set`, `list_push` (one element; doubles when full), `list_append`/`list_duplicate` (source is any `slice<T>` run: a list borrows in with `as`), `list_from_array` (copy a raw array), and `list_it`/`list_next` iteration.                                                                  |
...
| [string.mc](string.mc)                 | `import "string";`         | Growable text string `string` (a `type string = list<char>`): `string_init`/`string_duplicate`/`string_destroy`/`string_reset`, `string_get`/`string_set`, `string_push` (one char), `string_append`/`string_eq`/`string_duplicate` (right side is any `slice<char>` run: a string borrows in with `as`, a literal adapts directly), `string_from_array`/`string_append_array` (NUL-terminated `char*` sources), and `string_it`/`string_next` iteration. |
