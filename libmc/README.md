...
| ---------------------- | ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| [equality.mc](equality.mc) | `import "equality";` | The equality protocol's baseline overload set: `equals<T>(const self: slice<T>, const str: slice<T>) -> bool` compares two slices element by element (different lengths are never equal; empty slices compare equal; `T` must support `!=`). A string borrows in and a literal adapts, so `equals(s, "hi")` works directly. Open overload sets make it extensible: a type joins by adding an `equals` overload in its own module. |
