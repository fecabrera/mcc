...
| [std/io.mc](std/io.mc)       | `import "std/io";`    | Variadic `print` and `println` — printf-style output, forwarding through a `va_list` — plus the generic in-place helpers `swap` (exchange two values) and `replace` (store a value, return the old one), built on `mut` parameters.                                                                                                                                                                                                                        |
