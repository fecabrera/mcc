...
| [std/format.mc](std/format.mc) | `import "std/format";` | The formatting protocol's baseline overload set: every `format(mut str: string, value: X, const modifier: slice<char>)` member appends `value`'s rendering to `str` (a bare string literal adapts to the modifier, so `format(s, 255 as int32, "x")` works), with closed signed/unsigned integer groups (`x`/`X`/`p` hex and pointer modifiers; narrow signed widths sign-extend into the `int64` worker), `float64`, `bool` (`y`/`yes`), `char`/`char*`/`slice<char>` as text, `slice<char*>` as a quoted list, `slice<T>` as a bracketed per-element list, and an unbounded `<typename>` fallback. Open overload sets make it extensible: one `format` overload in your own module makes your type printable (see [Formatting](../docs/language.md#formatting)). |
...
| [std/io.mc](std/io.mc)       | `import "std/io";`    | Variadic `print` and `println` — `{[modifiers]}` placeholders rendered type-driven through the `std/format` overload set ([Formatted print/println](../docs/language.md#formatted-print--println)); the legacy printf-style pair stays behind `-D PRINTF_PRINTLN=1` — plus `writestr`/`writeln`/`writechar` and the generic in-place helpers `swap` (exchange two values) and `replace` (store a value, return the old one), built on `mut` parameters.                                                                                                                                                                                                                        |
